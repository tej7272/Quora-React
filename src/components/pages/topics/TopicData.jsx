...
const TopicData = (props) => {
  ...
  const { source,  title, body, date, image} = props;
    ...
    <Box sx={{ mt: "11px" }}>
      ...
      sx={{ 
        ...
        <CardHeader
          ...
          title={<Typography sx={{fontSize:'13px', fontWeight:'600'}}>{source}</Typography>}
        ...
        <CardContent>
          ...
          <Typography variant="body2" sx={{fontSize:'14px', color:`${darkMode?'#b8b4b4':'#4d4c4c'}` }}>{body}</Typography>
        </CardContent>
        <CardMedia
          component="img"
          height="350"
